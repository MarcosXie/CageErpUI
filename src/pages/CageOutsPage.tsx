...
const emptyForm: CageOutIdDto = { unitId: '', isActive: true }
...
export default function CageOutsPage() {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      if (editingId) {
        await updateCageId(editingId, formData)
      } else {
        const created = await createCageId(formData)
      ...
      if (createdIdentifier) {
        setGeneratedIdentifier(createdIdentifier)
      }
    } catch (requestError) {
    ...
    <section>
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex flex-wrap items-end gap-2">
          ...
          <button type="button" onClick={() => { setEditingId(null); setEditingIdentifier(null); setFormData({ ...emptyForm, unitId: unitFilter }); setIsModalOpen(true) }} disabled={isLoading || units.length === 0} className="flex h-10 items-center gap-2 border border-[#1f6553] bg-[#1f6553] px-3 text-sm font-semibold text-white hover:bg-[#123d33] disabled:opacity-60"><Plus size={17} /> Novo Cage ID</button>
        </div>
      </div>
